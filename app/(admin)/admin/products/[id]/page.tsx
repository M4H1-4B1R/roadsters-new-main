...
export default function EditProductPage({
  ...
  useEffect(() => {
    async function load() {
      const [prodResult, options] = await Promise.all([
        getProductAction(Number(id)),
        getCategoryOptions(),
      ]);
      if ("error" in prodResult) {
        setError(prodResult.error ?? "An unknown error occurred");
      } else {
        setProduct(prodResult.product as Record<string, unknown>);
      }
